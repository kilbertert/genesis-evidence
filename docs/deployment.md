...
| 论文证据审核工作台 | `127.0.0.1:10006` | 10006 | 论文采集/抽取/审核/知识卡(主线 A);**只绑环回** |
...
>
> **`10006` 已于 2026-09-24 收回环回**,依据是对该服务近两周访问日志的观测:
> 外部来源 **0 次**(全部命中来自本平台自身出口地址,即验收脚本所在主机),而同一次测量中
> 患者门户 `10007` 有至少 6 个不同的外部地址。审核工作台是内部工具,没有公开使用场景;
> 后续远程访问走独立通道(SSH 隧道或私有平面),不恢复公网绑定。详见
> `ops/service-host/README.md` 的 Exposure 一节。
