...
| 服务 | 入口 | 端口 | 说明 |
|---|---|---|---|
| 个人报告门户 | `http://<服务宿主>:10007` | 10007 | Health-Flow 体检报告解读与健康风险提示(主线 B);**绑 `0.0.0.0`,有真实外部用户** |
| 论文证据审核工作台 | `127.0.0.1:10006` | 10006 | 论文采集/抽取/审核/知识卡(主线 A);**目标为只绑环回,见下** |
...
>
> **`10006` 的暴露已决定收回,依据是对该服务近两周访问日志的观测**:外部来源 **0 次**
> (全部命中来自本平台自身出口地址,即验收脚本所在主机),而同一次测量中患者门户 `10007`
> 有至少 6 个不同的外部地址。审核工作台是内部工具,没有公开使用场景;后续远程访问走
> 独立通道(SSH 隧道或私有平面),不恢复公网绑定。
>
> **状态**:该决定已写入本仓库,但**主机侧的环境变量与重启尚未执行**——在主机上的
> `GENESIS_EVIDENCE_REVIEW_HOST` 改回 `127.0.0.1` 并重启 `genesis-evidence-review.service`
> 之前,该监听**实际仍在 `0.0.0.0`**。本表"入口"一列描述的是目标状态,不是当前状态;
> 执行前不要据它判断暴露面。详见 `ops/service-host/README.md` 的 Exposure 一节。
